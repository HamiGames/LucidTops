...
from builderMasterServer import DEFAULT_DOCKER_SERVICES, _get_mongo_client_for_launch, build_master_server
from config import (
    CONTAINER_ONION_DIR,
    LUCID_TOPS_ROOT,
    MASTER_SERVER_PORT,
...
DEFAULT_GUI_BRIDGE_PORT = 8105
DEFAULT_MONGODB_PORT = 27017
DEFAULT_DOCKER_NETWORK = "lucid-stack"
...
def _build_host_torrc(config: LaunchConfig) -> Path:
    ...
    lines = [
        ...
        f"HiddenServicePort 80 127.0.0.1:{config.master_server_port}",
        "",
        f"HiddenServiceDir {paths['frontend'].as_posix()}",
        "HiddenServiceVersion 3",
        f"HiddenServicePort 80 127.0.0.1:{config.gui_bridge_port}",
        "",
